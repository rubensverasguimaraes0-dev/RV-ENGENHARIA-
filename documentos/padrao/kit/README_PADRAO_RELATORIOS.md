...
└── referencia/
    ...
    ├── dados/obra_selecta.json      ← base completa da obra (equipe, semanas, 40 lançamentos, 8 semanas)
...
10. **Outros consumos do dia (extras)** — gelo, taxa de entrega de quentinha etc. — são linhas próprias, nunca somadas dentro da quentinha. No resumo da semana ficam agrupados por descrição, com dias, quantidade × unitário e custo, abaixo do subtotal de quentinhas; a linha final passa a ser `TOTAL — ALIMENTAÇÃO E OUTROS`.
11. **Feriado com expediente até meio-dia** = diária integral e sem quentinha (mesma regra do sábado).
12. **Uma página, sempre.** Quando a semana é grande (muitos funcionários, extras, muitas semanas no acumulado), o gerador reduz proporcionalmente a altura das linhas e o tamanho das fontes (fator `k` em `gerar_pdf`) em vez de criar uma segunda página.
...
- `extras` (opcional): outros consumos do dia, cada um com `descricao`, `qtd` e `valor` unitário. Exemplos reais: `{"descricao": "Gelo", "qtd": 2, "valor": 6.0}` e `{"descricao": "Entrega de quentinhas", "qtd": 1, "valor": 10.0}`. Entram no total do dia, aparecem como linha própria no bloco do dia e, no resumo da semana, agrupados por descrição abaixo das quentinhas. "Alimentação" nos totais = quentinhas + extras.
- `bonus` (opcional, no dia da conclusão de um serviço por produção): `{"descricao": "...", "diarias": {"Nome": 2, ...}}`. Cada pessoa recebe diárias × sua diária cheia; entra em mão de obra como linha própria ("Bônus de produção: N pessoas × D diárias"), não conta como presença, e no bloco por funcionário aparece como "3 + 2 bônus" com o total a pagar já somado.
- `semanas`: segunda a sábado; o relatório usa só os dias que existem em `lancamentos`.
- Semana em andamento: rode o gerador com `--parcial` — o título recebe "· PARCIAL" e a planilha "(PARCIAL)".
...
| **Acumulado** | | **108,5** | **R$ 16.995,00** | **105** | **R$ 2.036,00** | **R$ 19.031,00** |

\* Semana 8: R$ 288,00 de quentinhas (16 × R$ 18) + R$ 48,00 de gelo (8 pacotes × R$ 6) + R$ 20,00 de entregas (2 × R$ 10).
...
| Setembro (parcial, até 11/09) | 9 | 25 | R$ 3.740,00 | R$ 500,00 | R$ 4.240,00 |
